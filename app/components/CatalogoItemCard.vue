<template>
	<NuxtLink
		:to="`/catalogo/${item.id}`"
		class="block overflow-hidden rounded-xl bg-white shadow-lg transition-transform duration-300 hover:scale-105">
		<img
			v-if="item.imagenes?.[0]"
			:src="item.imagenes[0]"
			class="h-40 w-full cursor-zoom-in object-cover"
			@click.stop.prevent="showLightbox = true" />
		<div v-else class="flex h-40 w-full items-center justify-center bg-gray-100 text-gray-400">
			<Icon name="fa6-solid:image" class="text-3xl" />
		</div>

		<div class="p-4">
			<h3 class="mb-1 text-lg font-semibold text-gray-900">{{ item.nombre }}</h3>
			<p class="mb-2 text-sm text-gray-600">{{ item.usuario?.nombre }} {{ item.usuario?.apellido }}</p>
