<template>
	<div class="relative flex flex-col items-center justify-center gap-4 overflow-hidden rounded-xl bg-white p-6 shadow-lg transition-transform duration-300 hover:scale-105 dark:bg-neutral-800 dark:text-white">
		...
		<span class="text text-gray-600 dark:text-gray-300">{{ text }}</span>
		<div
			v-if="icon"
			class="absolute top-1 right-1 flex items-center justify-center rounded-full bg-black p-2 text-white">
			<Icon :name="icon" />
		</div>
	</div>
</template>
