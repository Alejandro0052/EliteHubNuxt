<template>
	<div class="mx-auto w-full max-w-[120rem] px-4 py-10">
		...
		<div class="mb-8 rounded-2xl bg-white p-6 shadow-md">
			<div class="flex flex-col items-center space-y-4 md:flex-row md:space-y-0 md:space-x-6">
				...
				<div class="flex-1 text-center md:text-left">
					...
					{{ user?.information?.profession }}
				</p>
					<div v-if="user?.information?.specialty" class="mt-2">
					<span class="inline-flex items-center rounded-full px-3 py-1 text-sm font-medium">
						{{ user.information.specialty }}
					</span>
				</div>
				</div>
		...
		<div v-if="user?.information?.experience" class="mt-6 rounded-2xl bg-white p-6 shadow-md">
			<h2 class="mb-4 text-xl font-semibold text-gray-900">Experiencia</h2>
			<div class="prose max-w-none text-gray-600">
				<p>{{ user.information.experience }}</p>
			</div>
		</div>
	</div>
</template>
